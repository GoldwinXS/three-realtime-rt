...
## Addendum (2026-08-12, owner field reports): three named repro fences

1. CORNELL-ROTATE HISTORY LEAK. Look at the cornell interior head-on, then
   rotate fast to the dark OUTSIDE of a side wall: the outside face glows
   with smeared interior history for a beat (worse at high maxHistory).
   Suspect: temporal validation accepts the inside->outside flip - a thin
   wall keeps plane distance small, and a normal test using abs(dot) (or a
   loose threshold) passes a 180-degree flip. Fence: strict signed
   rejection plus the Part 3 disocclusion fallback; the repro is scripted
   (head-on 1s, quarter-turn in 200ms, capture next 10 frames).
2. MOVING-LIGHT WISPY TAIL. A moving point light drags a wispy trail:
   stale EMA plus ReSTIR temporal reservoirs still holding the light's old
   position (reservoir reuse validates geometry, not light motion). Fence:
   scripted orbiting light, trail must decay within ~5 frames; check the
   reservoir refresh path (pHat revalidation) before inventing new
   machinery.
3. GOVERNOR BLIND AT VSYNC. On a 3060 the museum can run high res but
   lighting sits at boot-level 37.5 percent: at 60Hz vsync "fps at target"
   cannot distinguish barely-60 from could-do-200, so adaptiveQuality
   never climbs. Fireflies and frost live disproportionately in that
   low-res regime (the owner sees neither at high res on desktop or
   macbook). Fix direction: drive the governor on measured frame TIME
   headroom, not fps, and let it raise lighting res when there is budget.
   This is a separate engine item; do not fold it into the accumulation
   split leg, but its existence reframes priorities: the split must not
   regress the high-res path it will mostly not be needed in.

Also recorded: owner A/B'd restirGI against default GI at one light and at
full party lights; default GI won both on look and cost. restirGI stays
experimental and off; demote its panel copy rather than tuning it this
round.
